...
const checkUserJoinedChannels = async (userId) => {
  const channels = ['@Hamzaofficial786', '@Hamzabanproof'];
  let allJoined = true;
...
const sendChannelsRequiredMessage = async (chatId) => {
  return bot.sendMessage(chatId,
    `🚨 *You must join our official channels before pairing.*`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '📢 Channel 1', url: 'https://t.me/Hamzaofficial786' }],
          [{ text: '📢 Channel 2', url: 'https://t.me/Hamzabanproof' }],
...
bot.onText(/\/start/, async (msg) => {
  ...
  await bot.sendPhoto(
    ...
    {
      caption: `🪀 *𝙏𝙝𝙚 Hamza 𝑴𝑫💀*\n\n╔════════════════════╗\n ⤷ /pair <wa_number>\n ⤷ /unpair <wa_number>\n╚════════════════════╝`,
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
          [{ text: "👑 Owner", url: "https://t.me/Hamzahacr" }]
        ]
      }
    }
  );
...
bot.onText(/\/pair(?:\s+(.+))?/, async (msg, match) => {
  ...
  // 🔥 PRIVATE CHAT MEIN DIRECT PAIRING PROCESS (NO CHANNEL/GROUP JOIN REQUIRED)
if (!text) {
